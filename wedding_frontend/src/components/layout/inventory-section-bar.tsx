...
import { inventoryNavigationLeaves } from "@/lib/constants/navigation";
import { cn } from "@/lib/utils";

const isPathWithinHref = (pathname: string, href: string) =>
  pathname === href || pathname.startsWith(`${href}/`);

export function InventorySectionBar() {
  const location = useLocation();
  const { i18n, t } = useTranslation();
  const isRtl = i18n.resolvedLanguage === "ar";

  const activeHref = useMemo(() => {
    const matches = inventoryNavigationLeaves
      .filter(
    ...
    >
      ...
      >
        <div className="flex min-w-max flex-nowrap items-center gap-0 md:w-full md:min-w-0">
          {inventoryNavigationLeaves.map((item) => {
            if (!item.href) {
              return null;
            }
