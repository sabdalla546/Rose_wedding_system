...
export type QuotationDetailsWorkspaceProps = {
  quotationId?: string | number;
  onOpenQuotationEdit?: (quotationId: number) => void;
  onOpenContractCreate?: (quotationId: number) => void;
  onOpenContract?: (contractId: number) => void;
  onOpenEvent?: (eventId: number) => void;
  onDeleteSuccess?: (quotationId: number) => void;
};

export function QuotationDetailsWorkspace({
  quotationId,
  onOpenQuotationEdit,
  onOpenContractCreate,
  onOpenContract,
  onOpenEvent,
  onDeleteSuccess,
}: QuotationDetailsWorkspaceProps) {
  const { t, i18n } = useTranslation();
  const navigate = useNavigate();
  const { toast } = useToast();
  const routeParams = useParams();
  const resolvedQuotationId = quotationId ?? routeParams.id;
  const handleOpenQuotationEdit =
    onOpenQuotationEdit ??
    ((nextQuotationId) => navigate(`/quotations/edit/${nextQuotationId}`));
  const handleOpenContractCreate =
    onOpenContractCreate ??
    ((nextQuotationId) =>
      navigate(
        `/contracts/create?mode=from-quotation&quotationId=${nextQuotationId}`,
      ));
  const handleOpenContract =
    onOpenContract ?? ((contractId) => navigate(`/contracts/${contractId}`));
  const handleOpenEvent =
    onOpenEvent ?? ((eventId) => navigate(`/events/${eventId}`));
  const { data: quotation, isLoading } = useQuotation(
    resolvedQuotationId ? String(resolvedQuotationId) : undefined,
  );
  const { data: downstreamContractsResponse } = useContracts({
    currentPage: 1,
    itemsPerPage: 200,
    searchQuery: "",
    quotationId: String(resolvedQuotationId ?? ""),
    eventId: "",
    status: "all",
    signedDateFrom: "",
    signedDateTo: "",
  ...
  if (latestDownstreamContract) {
    nextSteps.push({
      ...
      action: (
        <Button
          type="button"
          variant="outline"
          onClick={() => handleOpenContract(latestDownstreamContract.id)}
        >
    ...
    nextSteps.push({
      ...
      tone: "default",
      action: (
        <Button type="button" onClick={() => handleOpenContractCreate(quotation.id)}>
              ...
              <>
                ...
                <ProtectedComponent permission="contracts.create">
                  <Button
                    ...
                    onClick={() => handleOpenContractCreate(quotation.id)}
                  >
                    <FilePlus2 className="h-4 w-4" />
                    {t("quotations.createContract", {
                      defaultValue: "Create Contract",
                ...
                <ProtectedComponent permission="quotations.update">
                  <Button
                    ...
                    onClick={() => handleOpenQuotationEdit(quotation.id)}
                  >
            ...
            items={[
              {
                ...
                value: (
                  <button
                    type="button"
                    onClick={() => handleOpenEvent(quotation.eventId)}
          ...
          deleteMutation.mutate(
            {
              ...
              redirectToList: !onDeleteSuccess,
            },
            {
              onSuccess: () => onDeleteSuccess?.(quotation.id),
            },
          )
        }
