...
const EventDetailsPage = () => {
  ...
  const [deleteCandidate, setDeleteCandidate] = useState<EventSection | null>(null);
  ...
  const createEventServiceMutation = useCreateEventService(Number(id || 0));
      ...
      sortedEventQuotations.reduce(
        (summary, quotation) => ({
          ...
          totalAmount: Number(
            (
              summary.totalAmount +
              (toNumberValue(quotation.totalAmount) ?? 0)
            ).toFixed(3),
          ),
        }),
        {
      ...
      sortedEventContracts.reduce(
        (summary, contract) => ({
          ...
          totalAmount: Number(
            (
              summary.totalAmount +
              (toNumberValue(contract.totalAmount) ?? 0)
            ).toFixed(3),
          ),
        }),
        {
  ...
  useEffect(() => {
    if (!serviceDialogOpen) {
      ...
      setServiceError("");
      setServiceForm(createDefaultEventServiceState(sortedEventServiceItems.length));
      return;
    }
  ...
  }, [editingServiceItem, serviceDialogOpen, sortedEventServiceItems.length]);
  ...
  const handleVendorSave = () => {
    ...
    const selectedVendor = vendorCatalog.find(
      (vendor) => String(vendor.id) === vendorForm.vendorId,
    );
    const companyNameSnapshot =
      vendorForm.companyNameSnapshot.trim() || selectedVendor?.name || "";
  ...
  const handleServiceSave = () => {
    ...
    createEventServiceMutation.mutate(payload, {
      onSuccess: () => setServiceDialogOpen(false),
    });
  };
    ...
    <>
                      ...
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        ...
                        <ProtectedComponent permission="events.update">
                          <div className="flex flex-wrap gap-2">
                            ...
                            <Button
                              variant="destructive"
                              onClick={() => setDeleteVendorCandidate(vendorLink)}
                            >
                              <Trash2 className="h-4 w-4" />
                              {t("common.delete", { defaultValue: "Delete" })}
                            </Button>
                      ...
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            <span className="rounded-full border border-[var(--lux-row-border)] bg-[var(--lux-panel-surface)] px-3 py-1 text-xs text-[var(--lux-text-secondary)]">
                              {t(`services.category.${serviceItem.category}`, {
                                defaultValue: formatServiceCategory(serviceItem.category),
                              })}
                            </span>
                            <span className="rounded-full border border-[var(--lux-row-border)] bg-[var(--lux-panel-surface)] px-3 py-1 text-xs text-[var(--lux-text-secondary)]">
                        ...
                        <ProtectedComponent permission="events.update">
                          <div className="flex flex-wrap gap-2">
                            ...
                            <Button
                              variant="destructive"
                              onClick={() => setDeleteServiceCandidate(serviceItem)}
                            >
                              <Trash2 className="h-4 w-4" />
                              {t("common.delete", { defaultValue: "Delete" })}
                            </Button>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <label className="space-y-2">
              <span className="text-sm font-medium text-[var(--lux-text)]">
                {t("events.sectionTypeLabel", { defaultValue: "Section Type" })}
              </span>
              <Select
              ...
              onValueChange={(value) =>
                setVendorForm((current) => {
                  ...
                  return {
                    ...current,
                    vendorId: nextValue,
                    companyNameSnapshot: selectedVendor?.name || current.companyNameSnapshot,
                  };
                })
              }
            >
      ...
      <ConfirmDialog
        ...
        message={t("vendors.deleteLinkMessage", {
          defaultValue: "Are you sure you want to delete this vendor assignment?",
        })}
        confirmLabel={t("common.delete", { defaultValue: "Delete" })}
        cancelLabel={t("common.cancel", { defaultValue: "Cancel" })}
        onConfirm={() =>
      ...
      <ConfirmDialog
        ...
        message={t("services.deleteEventItemMessage", {
          defaultValue: "Are you sure you want to delete this event service item?",
        })}
        confirmLabel={t("common.delete", { defaultValue: "Delete" })}
        cancelLabel={t("common.cancel", { defaultValue: "Cancel" })}
        onConfirm={() =>
