...
export type ContractDetailsWorkspaceProps = {
  contractId?: string | number;
  onOpenContractEdit?: (contractId: number) => void;
  onOpenQuotation?: (quotationId: number) => void;
  onOpenEvent?: (eventId: number) => void;
  onDeleteSuccess?: (contractId: number) => void;
};

export function ContractDetailsWorkspace({
  contractId,
  onOpenContractEdit,
  onOpenQuotation,
  onOpenEvent,
  onDeleteSuccess,
}: ContractDetailsWorkspaceProps) {
  const { t, i18n } = useTranslation();
  const navigate = useNavigate();
  const { toast } = useToast();
  const routeParams = useParams();
  const resolvedContractId = contractId ?? routeParams.id;
  const handleOpenContractEdit =
    onOpenContractEdit ??
    ((nextContractId) => navigate(`/contracts/edit/${nextContractId}`));
  ...
  const { data: contract, isLoading } = useContract(
    resolvedContractId ? String(resolvedContractId) : undefined,
  );
    ...
    nextSteps.push({
      ...
      action: (
        <Button
          type="button"
          variant="outline"
          onClick={() => handleOpenEvent(contract.eventId)}
        >
              ...
              <>
                ...
                <ProtectedComponent permission="contracts.update">
                  <Button
                    ...
                    onClick={() => handleOpenContractEdit(contract.id)}
                  >
            ...
            items={[
              {
                ...
                value: contract.quotationId ? (
                  <button
                    type="button"
                    onClick={() => handleOpenQuotation(contract.quotationId as number)}
      ...
      <ConfirmDialog
        ...
        onConfirm={() =>
          deleteMutation.mutate(
            ...
            {
              onSuccess: () => onDeleteSuccess?.(contract.id),
            },
          )
        }
